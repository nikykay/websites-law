...
/**
 * Add Custom Skins to the Posts widget
 */
function register_custom_post_skin($widget)
{
    //TODO Replace Custom_Skin and file name with your skin name
    include_once get_stylesheet_directory() . '/inc/elementor/class-custom-skin.php';
    $skins = array(
        'Custom_Skin',
    );
    ...
    foreach ($skins as $skin) {
        if (class_exists($skin)) {
            $widget->add_skin(new $skin($widget));
        }
    }
}

//add_action( 'elementor/widget/posts/skins_init', 'register_custom_post_skin' ); // Uncomment line to register custom post skin
...
{
    //TODO Replace Custom_Widget and file name with your widget name
    include_once get_stylesheet_directory() . '/inc/elementor/class-custom-widget.php';
    include_once get_stylesheet_directory() . '/inc/elementor/class-blog-info.php';
    include_once get_stylesheet_directory() . '/inc/elementor/class-latest-news.php';
    include_once get_stylesheet_directory() . '/inc/elementor/class-blogs-widget.php';
    include_once get_stylesheet_directory() . '/inc/elementor/class-service-widget.php';
    include_once get_stylesheet_directory() . '/inc/elementor/class-homepage-slider.php';
    $widgets = array(
        'Custom_Widget',
//        'Custom_Name'
        'Blog_Info',
...
add_action('elementor/widgets/widgets_registered', 'register_custom_widget'); // Uncomment line to register custom widget
...
function add_inline_style_after_custom()
{
    // Remove previous inline style
    $elementor_custom_styles = wp_styles()->get_data('elementor-frontend', 'after');
    wp_styles()->add_data('elementor-frontend', 'after', '');
    foreach ($elementor_custom_styles as $kit) {
        wp_add_inline_style('custom', $kit);
    }
}

// TODO Uncomment this line to resolve conflict if custom override elementor default typography styles
//add_action( 'wp_print_styles', 'add_inline_style_after_custom' );
...
function foundation_scripts_and_styles()
{
    if (!is_admin()) {
        ...
        if (is_plugin_active('elementor/elementor.php')) {
            // Check if elementor styles is used. If so - enqueue custom styles after it
            global $post;
            $elementor_active = !empty($post) ? \Elementor\Plugin::$instance->db->is_built_with_elementor($post->ID) : false;
            if ($elementor_active) {
            ...
            if (!$deps && is_plugin_active('elementor-pro/elementor-pro.php')) {
                ...
                if (!empty($headers) || !empty($footers)) {
                    $deps = array('elementor-frontend', 'elementor-pro');
                }
            }

            // Load Elementor Kit (global styles) on all pages
            $active_kit_id = \Elementor\Plugin::$instance->kits_manager->get_active_id();
            $css_file = \Elementor\Core\Files\CSS\Post::create($active_kit_id);

            if ($css_file->get_meta()['status'] === '') {
                $css_file->update();
            }

            if ($css_file->get_meta()['status'] == $css_file::CSS_STATUS_FILE) {
                wp_enqueue_style('elementor-post-' . $active_kit_id, $css_file->get_url(), $deps, null);
            }
        }

        //fonts
        wp_enqueue_style('google-fonts', 'https://fonts.googleapis.com/css?family=Open+Sans:400,400i,700&display=swap', null, null);

        //core
        wp_enqueue_style('foundation', get_template_directory_uri() . '/assets/css/foundation.css', $deps, '6.5.3');

        //system
        wp_enqueue_style('custom', get_template_directory_uri() . '/assets/css/custom.css', $deps, null);/*2rd priority*/
        wp_enqueue_style('style', get_template_directory_uri() . '/style.css', $deps, null);/*1st priority*/

        //fonts
        wp_enqueue_style('google-icons', 'https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@48,400,0,0', null, null);/*1st priority*/
...
function add_heading_new_line_support($html, $widget)
{
...
add_filter('elementor/widget/render_content', 'add_heading_new_line_support', 10, 2);
...
function change_font_display_value($font_display)
{
    return 'swap';
}

add_filter('elementor_pro/custom_fonts/font_display', 'change_font_display_value');
